...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE policyname = 'Assigned stakeholders can view orders'
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Assigned stakeholders can view order items'
...
-- Order Items: Allow assigned stakeholders to update items for their orders
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Assigned stakeholders can update order items'
      AND tablename = 'order_items'
  ) THEN
    CREATE POLICY "Assigned stakeholders can update order items"
    ON public.order_items
    FOR UPDATE
    USING (
      EXISTS (
