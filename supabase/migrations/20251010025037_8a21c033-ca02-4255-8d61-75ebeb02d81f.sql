-- Fix: Use passed-in actor_role and recompute state after concierge assignment
...
BEGIN
  ...
  -- Determine safe actor role (initial)
  SELECT CASE 
  ...
  -- Special handling for ready → delivered by concierge (race-safe)
  IF v_expected_lower = 'ready' 
     AND v_to_lower = 'delivered' 
     AND LOWER(p_actor_role) = 'concierge' THEN
    
    -- Race-safe assignment: only set if NULL
    UPDATE orders
    SET assigned_concierge_id = auth.uid()
    WHERE id = p_order_id
      AND assigned_concierge_id IS NULL;
    
    -- Re-read assignment so state is fresh in this txn
    SELECT assigned_concierge_id INTO v_assigned_concierge_id
    FROM orders WHERE id = p_order_id;
    
    -- Recompute safe role for accurate event logging
    SELECT CASE
      WHEN auth.uid() = assigned_shopper_id THEN 'shopper'
      WHEN auth.uid() = v_assigned_concierge_id THEN 'concierge'
      ELSE COALESCE(current_setting('app.current_user_role', true), 'system')
    ...
    -- Create checklist row (idempotent)
