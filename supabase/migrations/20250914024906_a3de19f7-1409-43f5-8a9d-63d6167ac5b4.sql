...
-- 3) Add shopper role to the current user
...
-- 5) Add some sample products and order items
DO $$
DECLARE
...
BEGIN
  ...
  IF sample_order_id IS NOT NULL THEN
    -- Create some basic products (without conflict handling)
    ...
    -- Add order items if products exist
    IF banana_product_id IS NOT NULL THEN
      INSERT INTO public.order_items (order_id, product_id, quantity, unit_price, total_price)
      VALUES (sample_order_id, banana_product_id, 2, 3.99, 7.98);
    END IF;
    
    IF milk_product_id IS NOT NULL THEN
      INSERT INTO public.order_items (order_id, product_id, quantity, unit_price, total_price)
      VALUES (sample_order_id, milk_product_id, 1, 4.99, 4.99);
    END IF;
    
    IF bread_product_id IS NOT NULL THEN
      INSERT INTO public.order_items (order_id, product_id, quantity, unit_price, total_price)
      VALUES (sample_order_id, bread_product_id, 1, 5.49, 5.49);
    END IF;
  END IF;
END $$;