-- Create workflow validation triggers and functions
...
-- Workflow transition audit trigger function
...
DROP TRIGGER IF EXISTS order_status_audit_trigger ON orders;
CREATE TRIGGER order_status_audit_trigger
  AFTER UPDATE ON orders
  FOR EACH ROW
  EXECUTE FUNCTION audit_workflow_transition();

-- Order assignment validation function
...
DROP TRIGGER IF EXISTS order_assignment_validation_trigger ON orders;
CREATE TRIGGER order_assignment_validation_trigger
  BEFORE UPDATE ON orders
  FOR EACH ROW
  EXECUTE FUNCTION validate_order_assignment();

-- Create function to rollback workflow status safely
...
-- Create comprehensive workflow integrity check function
